...
def crf_convert_examples_to_features(examples, tokenizer,  max_length=512, label_list=None, pad_token=0,  pad_token_segment_id=0, mask_padding_with_zero=True):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        attention_mask = attention_mask + (
                    [0 if mask_padding_with_zero else 1] * padding_length)  # [11110000] 1的部分是有内容的部分，0是padding的部分
...
def load_and_cache_example(args, tokenizer, processor, data_type):
    ...
    cached_features_file = "cached_{}_{}_{}".format(data_type, tokenizer.__class__.__name__, str(args.max_seq_length))  # cached_train_128
...
def trains(args, train_dataset, eval_dataset, model):
    ...
    no_decay = ['bias', 'LayerNorm.weight', 'transitions']  ## 梯度不衰竭
...
def default_logdir() -> str:
    ...
    return os.path.join("runs", datetime.now().strftime("%b%d_%H-%M-%S") + "_" + socket.gethostname())


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_dir", default=None, type=str, required=True, help="数据文件目录，因当有train.txt dev.txt")
    ...
    # Other parameters
    parser.add_argument("--pre_train_model", default=None, type=str, required=False, help="预训练的模型文件，参数矩阵。如果存在就加载")
    ...
    parser.add_argument("--logging_dir", default=os.path.join("runs", datetime.now().strftime("%b%d_%H-%M-%S") + "_" + socket.gethostname()), type=str, help="让学习增加到1的步数，在warmup_steps后，再衰减到0")
    ...
    # 得到tokenizer
    tokenizer_inputs = ()
    tokenizer_kwards = {'do_lower_case': False, 'max_len': args.max_seq_length, 'vocab_file': args.vob_file}
